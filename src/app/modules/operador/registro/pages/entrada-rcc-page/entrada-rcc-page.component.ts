...
@Component({
  ...
  templateUrl: './entrada-rcc-page.component.html',
})
export class EntradaRccPageComponent implements OnInit{
  ...
  public myFormRegistroRCC:FormGroup = this.fb.group({
    ...
    tramo: [""],                                        // Ok
    cui: [""],                                          // Ok
    cod_tpo_rgtro_dcmto: [2,[Validators.required]],     // Ok
    numOrdenServicio: [""],                             // Ok
  ...
  onSubmitDocumento(){
    ...
    if (this.myFormRegistroRCC.invalid ) {
      this.myFormRegistroRCC.markAllAsTouched();
      return;
    }
    // console.log(this.myFormRegistroRCC.value);
  ...
  resetFecha(){
    this.myFormRegistroRCC.get('fechaPlazo').setValue('')
  }
  ...
  responsableMostrar(event:any){
    ...
    if(id_oficina){
      const nomTrabajador = this.comboOfinasRcss.find((oficina) => oficina.iCodOficina == id_oficina)?.cNombresTrabajador;
      const apeTrabajador = this.comboOfinasRcss.find((oficina) => oficina.iCodOficina == id_oficina)?.cApellidosTrabajador;
      this.myFormRegistroRCC.get('nombreResponsbl').setValue(nomTrabajador,apeTrabajador)
    }else{
      this.myFormRegistroRCC.get('nombreResponsbl').setValue('')
    }
  }
  ...
  searchContrato(){
    const dialogRef = this.dialog.open(SearchContratosComponent, {
      disableClose:true,
      minWidth:900,
    })
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        const v1 = result.cui_parametro;
        const v2 = result.nro_contr_parametro;
        this.myFormRegistroRCC.patchValue({
          cui: v1,
          numeroContrato: v2
        })
  ...
  newOficinaDrvidadas(){
    const tipo_doc = this.myFormRegistroRCC.get('codigoTipoDocumento').value ? this.myFormRegistroRCC.get('codigoTipoDocumento').value : 19;
    const id_Tramite = this.id_tram;
